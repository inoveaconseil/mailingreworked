...
if (empty($sortfield)) $sortfield='type_template, position, label';
...
//Catégories de templates supplémentaires
$elementList['InformationLetter']=$langs->trans('InformationLetter');
$elementList['InfoAlert']=$langs->trans('InfoAlert');
$elementList['SendAgain']=$langs->trans('SendAgain');
$elementList['Invitation']=$langs->trans('Invitation');
    ...
    if (GETPOST('actionadd','alpha') || GETPOST('actionmodify','alpha'))
    {
        ...
        foreach ($listfield as $f => $value)
        {
            ...
            if ((! isset($_POST[$value]) || $_POST[$value]=='' || $_POST[$value]=='-1') && $value != 'lang' && $value != 'fk_user' && $value != 'position')
            {
                ...
                if ($fieldnamekey == 'libelle' || ($fieldnamekey == 'label'))  $fieldnamekey='ModeleName';
        ...
        if ($ok && GETPOST('actionadd'))
        {
            ...
            foreach ($listfieldinsert as $f => $value)
            {
            	...
            	if ($value == 'type_template') $_POST[$keycode]=$langs->trans($_POST[$keycode]);
...
$sql="SELECT rowid as rowid, label,  topic, type_template, private, position, content_lines, content, enabled, active";
...
foreach ($fieldlist as $field => $value)
{
	...
	if ($fieldlist[$field]=='code')            { $valuetoshow=$langs->trans("ModeleName"); }
	if ($fieldlist[$field]=='libelle' || $fieldlist[$field]=='label') { $valuetoshow=$langs->trans("ModeleName").'<span style="color:red">&nbsp;*</span>'; }
	if ($fieldlist[$field]=='type_template')   { $valuetoshow=$langs->trans("TypeOfTemplate").'<span style="color:red">&nbsp;*</span>'; }
...
foreach ($fieldsforcontent as $tmpfieldlist)
{
	...
	if ($tmpfieldlist == 'topic') {
		print '<strong>' . $form->textwithpicto($langs->trans("Topic"), $tabhelp[$id][$tmpfieldlist], 1, 'help', '', 0, 2, $tmpfieldlist) . '<span style="color:red">&nbsp;*</span></strong> ';
	}
...
print '</table>';
print '</div>';
print '<div style="margin-bottom: 4%"><input type="submit" class="button" name="actionadd" value="' . $langs->trans("Add") . '"></div>';
print '</form>';
print '<br>';
...
*/
    ...
    foreach ($fieldlist as $field => $value)
    {
		...
		*/
        ...
        if ($fieldlist[$field]=='type')            { $valuetoshow=$langs->trans("Type"); }
        if ($fieldlist[$field]=='libelle' || $fieldlist[$field]=='label') { $valuetoshow=$langs->trans("ModeleName"); }
    ...
    if ($num)
    {
        ...
        while ($i < $num)
        {
            ...
            if ($action == 'edit' && ($rowid == (! empty($obj->rowid)?$obj->rowid:$obj->code)))
            {
                ...
                if (! empty($conf->global->MAIN_EMAIL_TEMPLATES_FOR_OBJECT_LINES))
                {
                    $fieldsforcontent = array('topic', 'content', 'content_lines');
                }
                foreach ($fieldsforcontent as $tmpfieldlist)
                {
                	$showfield = 1;
                    ...
                    if ($showfield && ($tmpfieldlist != 'joinfiles')) {
...
function fieldList($fieldlist, $obj='', $tabname='', $context='')
{
	...
	foreach ($fieldlist as $field => $value)
	{
		...
		elseif ($context == 'edit' && in_array($fieldlist[$field], array('code', 'joinfiles', 'content', 'content_lines'))) continue;
